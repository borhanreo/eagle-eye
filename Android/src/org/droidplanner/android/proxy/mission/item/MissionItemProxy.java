...
public class MissionItemProxy {
	 ...
	 */
	List<LatLngDatabasesModel> latLonValue2 = new ArrayList<LatLngDatabasesModel>();
	public List<LatLong> getPath(LatLong previousPoint) {
		...
		switch (mMissionItem.getType()) {
			...
			case SPLINE_WAYPOINT:
				...
				//String testalt = String.valueOf(((MissionItem.SpatialItem) mMissionItem).getCoordinate().getAltitude());
				/*latLonValue2.add(new LatLngDatabasesModel(((MissionItem.SpatialItem) mMissionItem).getCoordinate().getLatitude(),
						((MissionItem.SpatialItem) mMissionItem).getCoordinate().getLongitude(),
						(int) ((MissionItem.SpatialItem) mMissionItem).getCoordinate().getAltitude()));*/
		...
		/*for(int i=0; i < latLonValue2.size(); i++){

		}*/

		SignletonLatLng.getInstance(context).setLatLngModel(latLonValue2);
		return pathPoints;
	}
